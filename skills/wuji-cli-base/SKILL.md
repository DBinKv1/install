...
| `update-required` | Installed Skills are older, or their version isn't readable. The next update syncs them |
| `not-installed` | No Wuji CLI Skills found. `wuji update` reports this state, and `wuji update --skills` installs them |

`wuji update --skills` syncs Skills without touching the binary, so you don't need a CLI upgrade to get Skills at the CLI's version or above. It exits 1 when Skills are still below it after the sync.
